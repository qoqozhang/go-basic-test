...
import (
	...
	"github.com/qoqozhang/go-basic-test.git/gorm_gen/model"
)
...
func (c companyDo) FilterWithNameAndRole(name string, role string) (result []model.Company, err error) {
	...
	generateSQL.WriteString("SELECT * from companies where name = " + c.Quote(name) + " and role = " + c.Quote(role) + " ")
...
func (c companyDo) FilterWithName(name string) (result []map[string]interface{}, err error) {
	var generateSQL strings.Builder
	generateSQL.WriteString("select * from companies where name = " + c.Quote(name) + " ")
